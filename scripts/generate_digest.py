...
def summarize_with_openai(items: list[NewsItem], config: Config) -> bool:
    ...
    body = json.dumps(
        {
            ...
            "instructions": (
                "Ты готовишь очень короткий ежедневный AI-дайджест. "
                "Верни строго JSON с массивом summaries. "
                "Каждой новости дай одну краткую русскоязычную выжимку, максимум 220 символов."
            ),
    ...
    try:
        ...
        summary_map = {entry["id"]: re.sub(r"\s+", " ", entry["summary"]).strip() for entry in parsed["summaries"]}
    except Exception as exc:  # noqa: BLE001
        print(f"[warn] OpenAI summarization failed: {exc}", file=sys.stderr)
        return False

    for item in items:
        item.summary = summary_map.get(item.fingerprint[:8], "")
    return True
